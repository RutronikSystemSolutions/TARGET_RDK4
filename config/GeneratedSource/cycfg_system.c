...
#define CY_CFG_SYSCLK_PLL0_BYPASS CY_SYSCLK_PLL_BYP_AUTO
#define CY_CFG_SYSCLK_PLL0_SOURCE CY_SYSCLK_PLL_SRC_ECO
#define CY_CFG_SYSCLK_PLL0_SOURCE_FREQ 12000000UL
#define CY_CFG_SYSCLK_SYSTICK_ENABLED 1
#define CY_CFG_SYSCLK_HFCLK_ENABLED 1
#define CY_CFG_SYSCLK_HFCLK_FREQ_MHZ 48UL
#define CY_CFG_SYSCLK_HFCLK_SOURCE CY_SYSCLK_CLKHF_IN_EXCO
...
static const cy_stc_sysclk_pll_manual_config_t exco_0_pll_0_pllConfig = 
{
    .feedbackDiv = 16U,
    .referenceDiv = 3U,
    .outputDiv = 0U,
...
{
    (void)Cy_GPIO_Pin_FastInit(GPIO_PRT0, 6, CY_GPIO_DM_ANALOG, 0UL, HSIOM_SEL_GPIO);
    (void)Cy_GPIO_Pin_FastInit(GPIO_PRT0, 7, CY_GPIO_DM_ANALOG, 0UL, HSIOM_SEL_GPIO);
    cy_en_sysclk_status_t status;
    status = Cy_SysClk_EcoConfigure(CY_CFG_SYSCLK_ECO_FREQ, 10UL, 50UL, 100UL);
...
{
    cy_en_sysclk_status_t status;
    status = Cy_SysClk_PllSetSource(0U, CY_CFG_SYSCLK_PLL0_SOURCE);
    if (CY_SYSCLK_SUCCESS != status)
    {
        cycfg_ClockStartupError(CY_CFG_SYSCLK_PLL_SRC_ERROR, status);
    }
    status = Cy_SysClk_PllManualConfigure(0U, &exco_0_pll_0_pllConfig);
    if (CY_SYSCLK_SUCCESS != status)
    {
        cycfg_ClockStartupError(CY_CFG_SYSCLK_PLL_CFG_ERROR, status);
    }
    Cy_SysClk_PllBypass(0U, CY_CFG_SYSCLK_PLL0_BYPASS);
    status = Cy_SysClk_PllEnable(0U, 250U);
    if (CY_SYSCLK_SUCCESS != status)
    {
...
{
    (void)Cy_SysClk_ClkPumpSetSource(CY_SYSCLK_PUMP_IN_HFCLK);
}
